...
try {
    ...
    $databaseReady = static function () use ($pdo): bool {
        $tables = (int) $pdo->query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name IN ('admin_users','articles','article_versions','geo_reviews','geo_proposals','builds','jobs','audit_events','schema_migrations','geo_scores','ai_bot_visits')")->fetchColumn();
        ...
        return $tables === 11 && $columns === 4 && $indexes === 2;
    };
