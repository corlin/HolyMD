...
try {
    $pdo->exec("UPDATE builds INNER JOIN jobs ON jobs.build_id = builds.id SET builds.status = 'queued', builds.started_at = NULL WHERE jobs.status = 'running' AND jobs.locked_at < DATE_SUB(UTC_TIMESTAMP(6), INTERVAL 15 MINUTE)");
    $pdo->exec("UPDATE geo_reviews INNER JOIN jobs ON jobs.geo_review_id = geo_reviews.id SET geo_reviews.status = 'queued' WHERE jobs.status = 'running' AND jobs.locked_at < DATE_SUB(UTC_TIMESTAMP(6), INTERVAL 15 MINUTE)");
    ...
    if ($job['job_type'] === 'build' && $job['build_id'] !== null) { $started = $pdo->prepare("UPDATE builds SET status = 'running', started_at = UTC_TIMESTAMP(6), completed_at = NULL, failure_message = NULL WHERE id = ? AND status IN ('queued', 'failed')"); $started->execute([$job['build_id']]); }
    if ($job['job_type'] === 'geo_review' && $job['geo_review_id'] !== null) { $started = $pdo->prepare("UPDATE geo_reviews SET status = 'running', completed_at = NULL, failure_message = NULL WHERE id = ? AND status IN ('queued', 'failed')"); $started->execute([$job['geo_review_id']]); }
    $pdo->commit();
...
try {
    ...
    if ($job['geo_review_id'] !== null) { $review = $pdo->prepare("UPDATE geo_reviews SET status = 'completed', completed_at = UTC_TIMESTAMP(6) WHERE id = ?"); $review->execute([$job['geo_review_id']]); }
    ...
    if ($job['build_id'] !== null) { $state = $pdo->prepare("UPDATE builds SET status = 'failed', failure_message = ?, completed_at = UTC_TIMESTAMP(6) WHERE id = ?"); $state->execute([substr($exception->getMessage(), 0, 1000), $job['build_id']]); }
    if ($job['geo_review_id'] !== null) { $state = $pdo->prepare("UPDATE geo_reviews SET status = 'failed', failure_message = ?, completed_at = UTC_TIMESTAMP(6) WHERE id = ?"); $state->execute([substr($exception->getMessage(), 0, 1000), $job['geo_review_id']]); }
