...
final readonly class EndpointPolicy {
 ...
 /** @return list<string> */ public function validate(string $url,bool $hasCredential): array {
  ...
  $host=trim($parts['host'],'[]');$addresses=filter_var($host,FILTER_VALIDATE_IP)?[$host]:($this->resolver!==null?($this->resolver)($host):$this->resolve($host));if($addresses===[])throw new InvalidArgumentException('GEO endpoint DNS resolution failed.');
  foreach($addresses as $address)if(!$this->isGlobalUnicast($address))throw new InvalidArgumentException('GEO endpoint must resolve only to public addresses.');return array_values(array_unique($addresses));
 }
