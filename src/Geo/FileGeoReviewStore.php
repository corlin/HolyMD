...
final class FileGeoReviewStore implements GeoReviewStore, GeoProposalStore {
    ...
    public function markAccepted(GeoProposalId $id, string $nextInputChecksum): void { $p = $this->get($id); foreach ($this->proposals as $candidate) if ($candidate->articleSlug === $p->articleSlug && $candidate->inputChecksum === $p->inputChecksum && $candidate->status === 'pending') $this->save(new GeoProposal($candidate->id, $candidate->articleSlug, $nextInputChecksum, $candidate->type, $candidate->value, $candidate->id->value === $id->value ? 'accepted' : 'pending', $candidate->bodyHash)); }
