...
final readonly class GeoConfiguration {
 ...
 public static function fromEnvironment(): self { $endpoint=(string)(getenv('HOLYMD_GEO_API_ENDPOINT')?:'https://api.openai.com/v1/chat/completions');$model=(string)(getenv('HOLYMD_GEO_MODEL')?:'gpt-5-mini');$configured=(bool)(getenv('HOLYMD_GEO_API_CREDENTIAL')&&getenv('HOLYMD_GEO_API_KEY'));$timeout=max(1,min(120,(int)(getenv('HOLYMD_GEO_TIMEOUT_SECONDS')?:30)));$max=max(1024,min(1048576,(int)(getenv('HOLYMD_GEO_MAX_RESPONSE_BYTES')?:262144)));return new self($endpoint,$model,$configured,$timeout,$max); }
