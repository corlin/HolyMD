...
final readonly class ConfiguredAiClient implements AiClient {
 ...
 public function analyze(string $systemPrompt,string $articleMarkdown): AiResponse {
  ...
  $body=json_encode(['model'=>$this->model,'messages'=>[['role'=>'system','content'=>$systemPrompt],['role'=>'user','content'=>$articleMarkdown]],'response_format'=>['type'=>'json_object']],JSON_THROW_ON_ERROR);
  ...
  $content=$payload['choices'][0]['message']['content']??null;if(!is_string($content)||trim($content)==='')throw new GeoAiException('GEO provider response did not contain structured review content.',true);return new AiResponse($content);
 }
}
