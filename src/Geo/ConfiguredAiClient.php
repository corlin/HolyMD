...
final readonly class ConfiguredAiClient implements AiClient {
 public function __construct(private string $credential,private string $endpoint,private string $model,private HttpTransport $transport=new StreamHttpTransport(),private int $timeoutSeconds=30,private int $maxResponseBytes=262144) {}
 public function analyze(string $systemPrompt,string $articleMarkdown): AiResponse {
  ...
  $schema=['type'=>'object','additionalProperties'=>false,'required'=>['proposals','findings'],'properties'=>['proposals'=>['type'=>'array','items'=>['type'=>'object','additionalProperties'=>false,'required'=>['type','value'],'properties'=>['type'=>['type'=>'string','enum'=>GeoReview::TYPES],'value'=>['oneOf'=>[['type'=>'string'],['type'=>'array','items'=>['type'=>'string']],['type'=>'object']]]]]],'findings'=>['type'=>'array','items'=>['type'=>'string']]]];
