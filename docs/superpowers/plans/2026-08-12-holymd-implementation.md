...
**Status:** completed and archived. This is a historical execution plan, not the current backlog.

**Closeout evidence:** all six task outcomes exist on `main`; the current release passes 284 tests / 1184 assertions, strict Composer validation, PHP/JavaScript syntax checks, deployment preflight, dry-run static build, real MySQL/Worker publication, and browser regression. Checkmarks below record delivered outcomes; they do not reconstruct the exact timestamp of every original RED/GREEN command.
...
- Public content is pre-generated and never rendered from Markdown or queried from MySQL per page. The default shared-host pointer resolver may stream the generated file through PHP, and AI-bot observability may record an anonymous event.
...
- [x] Write a failing test that boots with a test DSN and asserts the PDO error mode and required table names.
- [x] Run `vendor/bin/phpunit tests/BootstrapTest.php`; expect failure because the application classes and Composer setup do not exist.
- [x] Add Composer dependencies, PSR-4 autoloading, environment parsing, PDO bootstrap, and the complete idempotent schema.
- [x] Run `composer install` and `vendor/bin/phpunit tests/BootstrapTest.php`; expect PASS.
- [x] Commit with `git add composer.json public src database .env.example tests && git commit -m "chore: bootstrap HolyMD runtime"`.
...
- [x] Write tests for YAML parsing, required title/slug/date fields, safe path rejection, and body-preserving front-matter updates.
- [x] Run the focused PHPUnit tests; expect failures for missing parser/repository/builder classes.
- [x] Implement parser adapters, path-safe file access, templates, semantic HTML, canonical/meta tags, truthful Article/Person/WebSite/BreadcrumbList JSON-LD, RSS, JSON Feed, sitemap, robots, and optional `llms.txt` generation.
- [x] Add tests that build two articles and assert `/articles/slug/index.html`, feeds, sitemap, and JSON-LD content.
- [x] Run focused tests and `php -l` over `src/` and `templates/`; expect PASS.
- [x] Commit with `git add src/Content src/Render templates tests && git commit -m "feat: render markdown as static public site"`.
...
- [x] Write request tests for unauthenticated rejection, CSRF rejection, draft save without version creation, publish-only version advancement, and body round-trip.
- [x] Run the focused tests; expect failures for controllers, guard, and templates.
- [x] Implement server-rendered three-column writing studio with Markdown textarea, rendered preview, autosave endpoint, version list, and explicit Publish button; do not add AI prose actions.
- [x] Run focused tests plus PHP syntax checks; expect PASS.
- [x] Commit with `git add src/Http src/Auth src/Admin templates/admin public/assets tests && git commit -m "feat: add markdown writing studio"`.
...
- [x] Write tests with a fake AI client proving prompts forbid prose generation and that malformed responses are rejected.
- [x] Write an acceptance test that records the original body hash and fails if a proposal changes body Markdown.
- [x] Implement typed JSON response validation, review persistence, diff display, per-proposal accept/reject/edit, retryable job records, and encrypted/configured API credentials.
- [x] Run focused tests; expect PASS and explicit proof that accepted proposals update only front matter.
- [x] Commit with `git add src/Geo templates/admin/geo-panel.php tests/Geo && git commit -m "feat: add geo-only ai review"`.
...
- [x] Write tests that seed an existing live tree, force a renderer failure, and assert the live tree and manifest remain unchanged.
- [x] Write tests for slug changes producing redirects and withdrawn articles disappearing from feeds and sitemap.
- [x] Implement validation, temp-tree build, atomic pointer-file strategy supported by shared hosting, redirect generation, job claiming, and Cron-safe locking.
- [x] Run focused tests and execute `php bin/holymd-build.php --dry-run`; expect PASS and a readable validation report.
- [x] Commit with `git add src/Publish bin cron tests/Publish && git commit -m "feat: publish static site atomically"`.
...
- [x] Write end-to-end tests covering the complete publish flow and asserting AI cannot alter body content.
- [x] Run the full PHPUnit suite and syntax checks; expect failures until deployment configuration and tests are complete.
- [x] Add safe Apache rules for the admin entrypoint and pre-generated release serving, document backups of `content/`, media, database operational state, and generated public output.
- [x] Run `composer validate`, `vendor/bin/phpunit`, `find src public bin cron -name '*.php' -print0 | xargs -0 -n1 php -l`, and a clean dry-run build; expect PASS.
- [x] Commit with `git add README.md docs/operations public/.htaccess .env.example tests/EndToEnd && git commit -m "docs: verify shared hosting deployment"`.
...
This plan is complete. The active publication contract and later regression fixes are documented in the [2026-08-17 implementation plan](2026-08-17-publish-preflight-regression-hardening.md); future work must be recorded in a new dated plan rather than reopening these checkboxes.
