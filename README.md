...
- **不可变版本快照与一键回滚**：每次正式发布自动创建不可变快照，支持随时从历史版本恢复。
- **自定义单页管理 (Pages)**：支持关于页、条款页等独立单页的撰写、导航权重排序及静态构建。

### 2. 前台体验与现代发现体系
- **暗黑模式无闪烁切换**：原生 CSS 自定义属性驱动，支持系统跟随、浅色、深色三种模式，页面加载零闪烁。
- **智能目录 (TOC) 与阅读时间**：自动提取正文标题生成多层级目录（支持桌面侧边栏与移动端抽屉折叠），精准预估阅读用时。
- **画廊式图片弹窗预览 (Image Viewer)**：支持键盘快捷键导航（左右切换、`+`/`-` 缩放、`0` 还原、`Esc` 关闭）。
- **客户端毫秒级本地搜索**：轻量原生 JS 实时搜索，基于自动生成的静态索引，无需额外后端搜索引擎。
- **LLM / AI 智能体发现生态**：全站自动生成 `/llms.txt` 与包含全站公开文章正文的 `/llms-full.txt` 全文知识库。

### 3. AI GEO 边界与单项确认
- AI 的边界严格限定为 **GEO（Generative Engine Optimization）建议**：它可针对当前文章提出摘要、命名实体、FAQ 候选、元数据、内链与图片 Alt 建议，但**绝不自动篡改正文**。
- 每一项建议必须由管理员单独接受、编辑或拒绝；若正文在审核后发生变更，旧建议自动作废并要求基于新正文重新审核。
- 建议接受后直接转化为页面元数据、Article / FAQPage JSON-LD 结构化数据及公开关联链接。
...
# 运行全量单元测试与端到端测试 (249 tests, 989 assertions)
