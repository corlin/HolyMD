  ...
  <main class="reading-page"><article><div class="reading-layout shell">
    ...
    <aside class="toc-rail" aria-labelledby="desktop-toc-heading">
      <div class="toc-rail-inner">
        <p class="eyebrow"><?= __('In this essay') ?></p>
        <h2 id="desktop-toc-heading"><?= __('Contents') ?></h2>
    ...
    <div class="article-page">
      ...
      <details class="toc-box">
        <summary><?= __('Table of Contents') ?></summary>
        <?php require __DIR__ . '/_toc.php'; ?>
      </details>
      <?php endif; ?>
      <div id="article-content" class="prose"><?= $contentHtml ?></div>
    <?php if ($sources !== []): ?><section aria-labelledby="sources-heading"><div class="article-section sources"><h2 id="sources-heading"><?= __('Sources') ?></h2><ol><?php foreach ($sources as $source): ?><li><span class="icon" aria-hidden="true">link</span><a href="<?= htmlspecialchars($source) ?>" rel="cite noopener noreferrer"><?= htmlspecialchars($source) ?></a></li><?php endforeach; ?></ol></div></section><?php endif; ?>
  ...
  <dialog class="image-viewer" aria-labelledby="image-viewer-title">
    <div class="image-viewer-shell">
      <header class="image-viewer-toolbar">
        <div class="image-viewer-heading"><span class="icon" aria-hidden="true">image</span><div><strong id="image-viewer-title"><?= __('Image preview') ?></strong><span class="image-viewer-count" aria-live="polite"></span></div></div>
        <div class="image-viewer-actions" role="group" aria-label="<?= __('Image controls') ?>">
          <button type="button" data-image-zoom="out" aria-label="<?= __('Zoom out') ?>" title="<?= __('Zoom out') ?>"><span class="icon" aria-hidden="true">zoom_out</span></button>
          <button type="button" class="image-viewer-scale" data-image-zoom="reset" aria-label="<?= __('Reset zoom') ?>" title="<?= __('Reset zoom') ?>">100%</button>
          <button type="button" data-image-zoom="in" aria-label="<?= __('Zoom in') ?>" title="<?= __('Zoom in') ?>"><span class="icon" aria-hidden="true">zoom_in</span></button>
          <a class="image-viewer-download" href="" download aria-label="<?= __('Download original image') ?>" title="<?= __('Download original') ?>"><span class="icon" aria-hidden="true">download</span></a>
          <button type="button" data-image-close aria-label="<?= __('Close image preview') ?>" title="<?= __('Close') ?>"><span class="icon" aria-hidden="true">close</span></button>
        </div>
      </header>
      <div class="image-viewer-stage">
        <button type="button" class="image-viewer-nav image-viewer-prev" data-image-nav="prev" aria-label="<?= __('Previous image') ?>"><span class="icon" aria-hidden="true">chevron_left</span></button>
        <div class="image-viewer-canvas"><img src="" alt=""></div>
        <button type="button" class="image-viewer-nav image-viewer-next" data-image-nav="next" aria-label="<?= __('Next image') ?>"><span class="icon" aria-hidden="true">chevron_right</span></button>
      </div>
